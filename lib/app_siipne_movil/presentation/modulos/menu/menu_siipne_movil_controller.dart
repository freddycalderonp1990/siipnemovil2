...
class MenuSiipneMovilController extends GetxController {
  ...
  void onInit() {
    ...
    fechaInicio = DateTime(hoy.year, hoy.month, 1).obs;

    fechaFin = DateTime(hoy.year, hoy.month, hoy.day).obs;
  ...
  Future<void> _inicializarPantalla() async {
    ...
    try {
      final Pendiente? pendiente = await verificarOperativoPendiente();
      ...
      debugPrint('ERROR INICIALIZANDO MENU SIIPNE: $e');
  ...
  Future<Pendiente?> verificarOperativoPendiente() async {
    try {
      ...
      final Pendiente pendiente = await siipneMovilUseCase.consultaPendiente(
  ...
  Future<void> getModulosPermitidos() async {
    try {
      ...
      final List<DataModulo> data = await siipneMovilUseCase.getModulos(
      ...
      debugPrint('ERROR CONSULTANDO MÓDULOS: $e');
      debugPrint('$stackTrace');
    } finally {
      peticionServerState.value = false;
    }
  ...
  void cambiarFechaInicio(DateTime fecha) {
    fechaInicio.value = DateTime(fecha.year, fecha.month, fecha.day);
  }

  void cambiarFechaFin(DateTime fecha) {
    fechaFin.value = DateTime(fecha.year, fecha.month, fecha.day);
  }
  ...
  Future<bool> consultarOperativosUsuario() async {
    if (consultandoOperativos.value || descargandoPdf.value) {
      return false;
    }
    ...
    try {
      ...
      final List<DataOperativosUsuario> resultado = await siipneMovilUseCase
          .consultarOperativosUsuario(request: request);

      /*
       * Por seguridad volvemos a ordenar desde el móvil.
       * El más reciente queda primero.
       */
      resultado.sort((DataOperativosUsuario a, DataOperativosUsuario b) {
        final DateTime? fa = DateTime.tryParse(a.fechaEvento);

        final DateTime? fb = DateTime.tryParse(b.fechaEvento);

        if (fa == null || fb == null) {
          return b.idHdrEvento.compareTo(a.idHdrEvento);
        }
      ...
      debugPrint('ERROR CONSULTANDO OPERATIVOS USUARIO: $e');
      debugPrint('$stackTrace');
  ...
  void limpiarConsultaOperativos() {
    ...
    fechaInicio.value = DateTime(hoy.year, hoy.month, 1);

    fechaFin.value = DateTime(hoy.year, hoy.month, hoy.day);
  }

  // ============================================================
  // PDF
  // ============================================================
  Future<String?> descargarPdfOperativo(DataOperativosUsuario operativo) async {
    if (descargandoPdf.value || consultandoOperativos.value) {
      return null;
    }

    if (operativo.idHdrEvento <= 0) {
      mensajeErrorPdf = 'El número del operativo no es válido.';

      return null;
    }

    if (user.idGenUsuario <= 0) {
      mensajeErrorPdf = 'No se pudo identificar al usuario autenticado.';
    ...
    idOperativoDescargando.value = operativo.idHdrEvento;

    try {
      debugPrint('==========================================');
      debugPrint('DESCARGANDO REPORTE OPERATIVO');
      debugPrint('ID USUARIO: ${user.idGenUsuario}');
      debugPrint('ID HDR EVENTO: ${operativo.idHdrEvento}');
      debugPrint('==========================================');

      final String filePath = await siipneMovilUseCase.downloadPdfOperativo(
        idGenUsuario: user.idGenUsuario,
        idHdrEvento: operativo.idHdrEvento,
      );

      if (filePath.trim().isEmpty) {
        mensajeErrorPdf = 'No fue posible obtener el archivo PDF.';
      ...
      mensajeErrorPdf = 'No fue posible descargar el reporte del operativo.';
